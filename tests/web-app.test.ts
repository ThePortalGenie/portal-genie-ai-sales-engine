...
test("GET /assets/app.js includes commercial analysis action", async () => {
  await withServer(async (base) => {
    ...
    assert.match(js, /candidates_awaiting_analysis/);
    ...
    assert.match(js, /isPrimaryQueueExcluded/);
    assert.match(js, /data-watch-id/);
    assert.match(js, /focusWatchItem/);
    assert.match(js, /Nothing needs immediate customer contact/);
    assert.match(js, /appendKv\(body, "Timing"/);
    assert.match(js, /Why this client matters/);
    assert.doesNotMatch(js, /renderDailyBrief/);
    assert.doesNotMatch(js, /appendKv\(card, "Confidence"/);
    assert.doesNotMatch(js, /cc-filters/);
